                                                ...
                                                <section className='flex flex-row justify-between'>
                                                    ...
                                                    {/* <h1 className='text-sm'>
                                                        {newMessages[data] && typeof newMessages[data] === 'object'
                                                            ? (Object.keys((newMessages as NewMessages)[data]).pop() && (newMessages as NewMessages)[data][Object.keys((newMessages as NewMessages)[data]).pop() as string]?.text[0])
                                                            : ""}
                                                    </h1> */}
                                                </section>
                                            </div>
                                    ...
                                    <div className=''>
                                        <div className='h-[400px] messageArea'>
                                            {selected.roomId && newMessages[selected.roomId]
                                                &&
                                                Object.values(newMessages[selected.roomId]).sort((a: any, b: any) => b.timestamp - a.timestamp).map((message: any, i: number) => (
                                                    <div
                                                        key={i}
                                                ...
                                                ${message.senderId === userId ? 'ml-auto mr-2 rounded-bl-[6px] rounded-br-[6px] rounded-tl-[6px]'
                                                    ...
                                                    >
                                                        ...
                                                        {/* {message.images ? <img src={URL.createObjectURL(message.images)} alt="Your Alt Text" className='h-32 w-32' /> : ''} */}
                                                    </div>
                                                ))}
                                        </div>
                                        {image.length > 0 && <div className='flex flex-row p-2 space-x-5  bg-gray-100'>
                                            {image.map((img: any, i: number) => (
                                                <div className='flex justify-center h-40 w-32' key={i}>
                                                    {/* eslint-disable-next-line @next/next/no-img-element */}
                                                    <img src={URL.createObjectURL(img)} alt="Your Alt Text" className='h-32 w-32' />
                                        ...
                                        <div className='border-none rounded-br-[7px] bg-gray-100 h-14 p-2 flex flex-row px-8 space-x-3'>
                                            <div className='w-auto cursor-pointer'>
                                                <CameraAlt className='text-2xl mt-2' onClick={() => handleCamera()} />
                                                <input type="file" className='invisible' id='fileInput' ref={fileInputValue} onChange={(e) => handleImage(e)} multiple />
                                            </div>
                                            <form method='post' className='bg-white w-full flex flex-row border-none rounded-lg' onSubmit={(e) => handleMessage(e)}>
                                                {image.length > 0 ?
                                                    ''
                                                    :
                                                    <div className='w-full p-2'>
                                                        <input type="text" required placeholder='Type a message' value={message} className='focus:outline-none w-full h-auto' onChange={(e) => setMessage(e.target.value)} />
                                                    </div>
                                                }
                                            </form>
                                            <button className='bg-[#FF0000] w-16 flex justify-center cursor-pointer border-none rounded-br-md rounded-tr-md' onClick={(e) => handleMessage(e)}>
